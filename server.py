...
import random
import base64
import uuid  # Used to prevent file collisions during concurrent requests
...
# Determine the correct temp directory based on OS
...
def solve_base64():
    ...
    # Generate unique filenames for this transaction
    request_id = str(uuid.uuid4())
    mp3_path = os.path.join(TEMP_DIR, f"voice_{request_id}.mp3")
    wav_path = os.path.join(TEMP_DIR, f"voice_{request_id}.wav")
        
    try:
        # Decode the Base64 audio string and write it to disk
        audio_bytes = base64.b64decode(data["audio_base64"])
        with open(mp3_path, "wb") as f:
            f.write(audio_bytes)
        
        # Load and process the decoded MP3 file
        ...
        # Securely delete local session files
