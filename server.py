...
app = Flask(__name__)
CORS(app)  # Allows browser extension to communicate with API safely

# --- CONFIGURATION OPTIONS ---
DEFAULT_SOLVE_MODE = "words"  # Options: "words", "numbers", "all"
# ----------------------------
...
def health_check():
    """Provides standard health & status check for Chrome extension popup pinging."""
    uptime_seconds = int(time.time() - START_TIME)
    return jsonify({
        "status": "online",
        "service": "AI Buster Solver KKH Pro Online Backend",
        "uptime_seconds": uptime_seconds,
        "default_solve_mode": DEFAULT_SOLVE_MODE,
        "version": "1.0.0"
    }), 200

@app.route("/solve_base64", methods=["POST"])
def solve_base64():
    """Receives Base64 audio data directly from extension."""
    data = request.get_json()
    if not data or "audio_base64" not in data:
        return jsonify({"error": "No audio data provided"}), 400
        
    solve_mode = data.get("solve_mode", DEFAULT_SOLVE_MODE)
    ...
    try:
        ...
        if solve_mode != "all" and challenge_type != solve_mode:
            ...
            return jsonify({"error": f"Skipped {challenge_type} challenge based on solve_mode '{solve_mode}'"}), 400
