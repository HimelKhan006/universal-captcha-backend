...
SOLVE_MODE = "words"  # Options: "words" (Skips/reloads any digit challenge)
# ----------------------------
...
def detect_challenge_type(text):
    """
    Highly aggressive classifier. If any number, digit, or digit-homophone 
    is detected, it classifies the challenge as 'numbers' to force a reload.
    """
    if not text:
        return "unknown"
    ...
    # Check if there are any actual digits (e.g., "3", "8")
    if any(char.isdigit() for char in cleaned_text):
        return "numbers"
        
    # Check if any word matches common digit homophones or written numbers
    number_keywords = {
        "zero", "one", "two", "three", "four", "five", "six", "seven", "eight", "nine",
        "oh", "to", "too", "for", "fore", "ate", "won"
    }
    if any(word in number_keywords for word in words):
        return "numbers"
...
def solve_base64():
    ...
    try:
        ...
        if SOLVE_MODE != "all" and challenge_type != SOLVE_MODE:
            print(f"[!] Mismatch: Skip numbers enabled. Requesting reload.")
